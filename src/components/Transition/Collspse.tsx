...
class Collapse extends Component<ICollapseProps> {
  ...
  revertStyle = node => {
    if (this.inlineStyle) {
      ...
      cssNames.boxProps.forEach(
        name => (node.style[name] = this.inlineStyle[name])
      );
  ...
  onEntering = node => {
    if (node) {
      ...
      const cssNames = this.getCssNames();
      cssNames.boxProps.forEach(
        name => (node.style[name] = this.defaultStyle[name])
      );
  ...
  public render() {
    ...
    return (
      <Transition timeout={timeout!} {...props} {...this.transitionEvents} />
    );
  }
}
